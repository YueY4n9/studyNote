...
4. 主从复制架构
   ...
   主从复制架构只是将主节点的数据同步到从节点，从节点默认只读；主节点挂掉之后从节点也不会替换主节点继续工作。这种架构并不常用，因为从节点顶不了；无法解决Redis高可用问题。

5. 哨兵（Sentinel）机制

   哨兵机制是Redis的高可用性解决方案：哨兵就是带有自动故障转移功能的主从架构。哨兵机制并不能解决并发压力和物理上限问题，因为始终都是一个主节点在工作；无法解决并发压力和物理上限问题。
   
   ![image-20210511000510638](C:\Users\YueYang\Documents\GitHub\StudyNote\img\image-20210511000510638.png)
   
   
   
   在sentinel.conf配置文件中填写：
   
   sentinel monior 哨兵名称 ip port 1
